...
static void update_sampling_rate(unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (time_before(next_sampling, appointed_at)) {
			...
			mutex_lock(&nightmare_cpuinfo->timer_mutex);
			queue_delayed_work_on(nightmare_cpuinfo->cpu, system_wq,
				&nightmare_cpuinfo->work, usecs_to_jiffies(new_rate));
		}
		mutex_unlock(&nightmare_cpuinfo->timer_mutex);
	}
	put_online_cpus();
...
static void nightmare_check_cpu(struct cpufreq_nightmare_cpuinfo *this_nightmare_cpuinfo)
{
	...
	if (wall_time >= idle_time) {
		...
		if (cur_load >= inc_cpu_load && cpu_policy->cur < max_freq) {
			tmp_freq = max(min((cpu_policy->cur + (
				(cur_load + freq_step - freq_up_brake == 0 ? 1 : cur_load + freq_step - freq_up_brake) * 3780)),
				max_freq), min_freq);
		} else if (cur_load < dec_cpu_load && cpu_policy->cur > min_freq) {
			tmp_freq = max(min((cpu_policy->cur - (
				(100 - cur_load + freq_step_dec == 0 ? 1 : 100 - cur_load + freq_step_dec) * 3780)),
				max_freq), min_freq);
		} else {
			tmp_freq = cpu_policy->cur;
		}
